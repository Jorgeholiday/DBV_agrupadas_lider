...
PASTA_DADOS = "db"
# CORREÇÃO: Apontando para a subpasta correta de PDFs do líder
PASTA_PDFS = os.path.join("pdfs", "pdfs_lider")
    ...
    for i, tarefa in enumerate(tarefas):
        ...
        if com_pdf:
            ...
            if not os.path.exists(caminho_pdf):
                print(f"   - ERRO: PDF não encontrado em '{caminho_pdf}'. Pulando anexo.")
            else:
...
if __name__ == "__main__":
    ...
    # Carrega usuários
    caminho_grupos = os.path.join(PASTA_DADOS, "grupo-lider.csv")
    usuarios = carregar_csv(caminho_grupos)
    
    # --- Lógica para receber escolhas da UI ou rodar interativamente ---
    if len(sys.argv) == 3:
        # Rodando a partir da UI: sys.argv[1] é o usuário, sys.argv[2] é o modo
        print("Recebendo comandos da interface gráfica...")
        try:
            escolha_usuario_num = int(sys.argv[1])
            modo_escolhido_num = int(sys.argv[2])
            
            if not (1 <= escolha_usuario_num <= len(usuarios)):
                print(f"ERRO: Número de usuário inválido: {escolha_usuario_num}")
                sys.exit(1)
            if modo_escolhido_num not in [1, 2]:
                print(f"ERRO: Modo inválido: {modo_escolhido_num}")
                sys.exit(1)
                
            usuario_escolhido = usuarios[escolha_usuario_num - 1]
            modo_escolhido = modo_escolhido_num
            print(f"Usuário selecionado: {usuario_escolhido['nome']}")
            print(f"Modo selecionado: {'SEM PDF' if modo_escolhido == 1 else 'COM PDF'}")

        except (ValueError, IndexError):
            print("ERRO: Argumentos inválidos recebidos da UI.")
            sys.exit(1)
    else:
        # Rodando manualmente (sem UI), mostra os menus como antes
        print("Rodando em modo interativo (sem interface gráfica)...")
